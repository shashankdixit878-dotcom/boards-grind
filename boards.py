...
{% if chapter_cards %}<div class="revision-grid">{% for c in chapter_cards %}<div class="revision-card"><div class="kpi"><div><b>{{ c.chapter }}</b><div class="small muted">{{ c.subject }}</div></div><div class="right"><b>{{ c.done_count }}/5</b><div class="small muted">{{ c.percent }}%</div></div></div><div class="progress"><div class="bar" style="width:{{ c.percent }}%"></div></div><div class="rounds">{% for r in c.rounds %}<span class="round {% if r.status=='Completed' %}done{% elif r.status=='Overdue' %}overdue{% elif r.status=='Due Today' %}today{% endif %}">{{ r.round_label }} · {{ r.due_date }}{% if r.status=='Completed' %} ✓{% endif %}</span>{% endfor %}</div><div class="btnrow"><form method="post" action="{{ url_for('reset_revision', chapter_id=c.chapter_id) }}" onsubmit="return confirm('Reset all 5 revision rounds for this chapter?');"><button class="btn btn2" type="submit">↻ Reset Revision</button></form></div></div>{% endfor %}</div>{% else %}<div class="card empty">No completed chapters yet. Complete a chapter from Subjects and its five revision rounds will appear here.</div>{% endif %}
"""
    return page("Revision", body, active="Revision", items=items, chapter_cards=chapter_cards, counts=counts, filter_name=filter_name)


@app.route("/revision/<int:chapter_id>/reset", methods=["POST"])
def reset_revision(chapter_id):
    """Reset only the five revision rounds for a completed chapter."""
    conn = db()
    chapter = conn.execute(
        "SELECT id FROM chapters WHERE id=? AND status='Completed'",
        (chapter_id,)
    ).fetchone()

    if chapter:
        conn.execute(
            "UPDATE revision_rounds SET completed_at=NULL WHERE chapter_id=?",
            (chapter_id,)
        )
        conn.commit()

    conn.close()
    return redirect(request.referrer or url_for("revision"))
